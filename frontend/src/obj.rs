pub mod components {
    use dioxus::prelude::*;
    static SERVICE: Atom<Service> = |_| Service::Splash;
                ...
                div {
                    ...
                    div {
                        ...
                        div {
                            ...
                            button {
                                onclick: move |_| {
                                    set_service(Service::Splash);
                                    cx.spawn({
                                        async move {
                                            let client = reqwest::Client::new();
                                            let res = handlers::send_service("None".to_string(), &client).await;
                                            match res {
                                                _ => {}
                                            }
                                        }
                            ...
                            ul {
                                ...
                                li {
                                    button {
                                        onclick: move |_| { 
                                            set_service(Service::Translate);
                                            cx.spawn({
                                                async move {
                                                    let client = reqwest::Client::new();
                                                    let res = handlers::send_service("Translation".to_string(), &client).await;
                                                    match res {
                                                        _ => {}
                                                    }
                                                }
                                ...
                                li {
                                    button {
                                        onclick: move |_| {
                                            set_service(Service::Summarize);
                                            cx.spawn({
                                                async move {
                                                    let client = reqwest::Client::new();
                                                    let res = handlers::send_service("Summarization".to_string(), &client).await;
                                                    match res {
                                                        _ => {}
                                                    }
                                                }
                                    ...
                                    textarea {
                                        ...
                                        oninput: move |req| {
                                            ...
                                            cx.spawn({
                                                ...
                                                async move {
                                                    let out = handlers::handle_prediction(&target, req.value.clone(), &client).await;
                                                    match out {
                                                        Ok(o) => {
                                                            sl(false);
                                    ...
                                    textarea {
                                        ...
                                        oninput: move |req| {
                                            ...
                                            cx.spawn({
                                                ...
                                                let client = reqwest::Client::new();
                                                async move {
                                                    let out = handlers::handle_summarization(req.value.clone(), &client).await;
                                                    match out {
                                                        Ok(o) => {
                                                            sl(false);
                                                            output.set(o.text().await.unwrap())
    ...
    pub fn NLP_service(cx: Scope) -> Element {
        ...
        cx.render(rsx!(
            div {
                ...
                match curr_service {
                    Service::Splash => cx.render(rsx!(
                        Splash {}
                    )),
                    Service::Summarize => cx.render(rsx!(
                        Summarization {}
                    )),
                    Service::Translate => cx.render(rsx!(
                        Translation {}
                    ))
                }
            }
    ...
    pub mod handlers {
        pub async fn send_service(service: String, client: &reqwest::Client) -> Result<reqwest::Response, reqwest::Error> {
        ...
        pub async fn handle_prediction(target: &String, query: String, client: &reqwest::Client) -> Result<reqwest::Response, reqwest::Error> {
            let mut map = std::collections::HashMap::new();
        ...
        pub async fn handle_summarization(query: String, client: &reqwest::Client) -> Result<reqwest::Response, reqwest::Error> {
            let mut map = std::collections::HashMap::new();
            map.insert("query", query);
